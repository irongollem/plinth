...
- [ ] Compressed-at-rest option: after cleanup, keep each model bundled on disk (pack/unpack on demand) to save space while staying workable — the finalize_release compression path is the seed tech
  ...
  - [ ] Phase 2 — transparent use: `ensure_model_files` selective extraction (print only the ticked files straight from the archive), "clean up extracted files after" checkbox (`pack_cleanup_after` setting already exists), render/3D-preview hooks, app-exit cleanup
  - [ ] Phase 3 — scale + polish: bulk "Pack models…" toolbar flow (designer facet → whole designer), pack.json self-heal from archive central dir, packed-path hints in the dup UI, real-NAS shakedown
