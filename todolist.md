...
- [ ] Base cutter phases 1–4 shipped 2026-07-14 (see [docs/BASECUTTER.md](docs/BASECUTTER.md)): the script (`resources/base_cut.py`, manifold-verified + slicer-checked), the cutter library (`basecutter/cutters.rs`, 24 sizes as data), the job pipeline (start/cancel + BaseCutStatus events, real-Blender integration test), and the tab + top-down placement viewport — phases 2–4 built by Sonnet subagents against the doc's pinned interfaces, reviewed per-phase; REMAINING before calling it v1: a human GUI run (pick landscape → place → cut → STL appears), print one base + press-fit a 5×1 magnet, then phase 5 polish (placement generators/regiments, magnet inventory in settings, validation surfacing, export-into-catalog)
