...
Open work is up top; everything shipped is under **Done**, grouped by area.
Done entries keep their full write-ups on purpose — the history is the design
record.
...
- [ ] Base cutter phases 1–4 + 6 shipped 2026-07-14 (see [docs/BASECUTTER.md](docs/BASECUTTER.md)): the cut script, cutter library (24 sizes as data), job pipeline, tab + placement viewport, PLUS a full /code-review pass (22 verified findings, all fixed same day: silent-crash exit codes, cancel race, STL-overwrite name collision, mesh leaks, real validation gate, Rust-owned cut derivation on the wire, shared Blender harness replacing three copies, overlay GPU leak, cancelled-vs-failed UX) and the phase-6 landscape generator (gen_landscape.py, six composable heightfield layers, cobblestone/sandy/rocky/lava presets as data, folded-advanced UI; a 32mm round cut from the cobblestone bake is manifold end-to-end); REMAINING before calling it v1: a human GUI run (generate → place → cut → STL appears; check drag feel, lock states, cancel toast), print one base + press-fit a 5×1 magnet, then phase 5 polish (regiments/random-scatter placement generators, magnet inventory in settings, export-into-catalog) and the further-horizon ideas (scatter, relief painting)
...
### Needs a real-NAS shakedown (manual validation — the code is shipped)

- [ ] Compressed-at-rest over SMB: pack one small designer → cancel mid-run → re-run resumes; kill the app mid-pack → restart converges; verify the hardlink probe + timestamps behave on the Synology volume
- [ ] Normalizer/cleaner over SMB: the same shakedown for the cleaner's moves — variant materialization + wholesale base-dir renames preserve hardlinks and converge on the case-sensitive volume before bulk use

### Parked (blocked on an upstream decision, not effort)

- [ ] Remaining catalog filter facets — year (from release date), release name, and category (new concept, still TBD): designer facet + grouping shipped 2026-07-05; revisit after the normalizer settles
- [ ] Selective DOWNLOAD of only changed/new components: presupposes a distribution channel (an HTTP host serving components) that doesn't exist yet; the local half — selective import of changed components — shipped (see Modular Package Strategy below)
- [ ] Bandwidth estimation + progress indicators for partial downloads: blocked on the same missing distribution story as selective download
- [ ] Duplicate handling Phase 5 — virtual sharing tier (only if real users are stuck on link-less volumes): catalog-pointer fallback, viable once print resolves paths in-app; Finder browsing is the only remaining gap

## Done

### Catalog, normalizer & cleanup contract
...
- [x] File-level variants materialize as variant folders (shipped in `32b7f36`, test `file_level_variants_materialize_as_folders`): a dump folder split onto FILES in the drawer has ONE member row with variant NULL — the old plan flattened everything into `Supported/` and the variants existed only as invisible metadata. Such members now fan out per FILE (`FanOutIntent`/`fanout_by_leaf`): each file's effective facets pick its `Supported|Unsupported/Variant` leaf, case-unified so "Sword"/"sword" don't fork on the case-sensitive NAS. (Real-NAS shakedown tracked under Open.)
- [x] Cleanup-tooling hardlink contract upheld (verified 2026-07-09): same-volume moves are `fs::rename` — a metadata op that preserves the inode and every hardlink (the dedup merge invariant). The cleaner REFUSES cross-volume moves loudly (EXDEV surfaces as a reported error, never a silent copy+delete that would split shared inodes). Deleting/merging only ever drops ONE name; the content survives in its twin. Re-establishing sharing after a user relocates a library ACROSS volumes is the dedup "Merge — free X" feature (`dups.rs` hard_links identical files on the destination). Each clause is individually tested (merge test in dups.rs; cross-volume refusal in normalize.rs).
...
- [x] Fast catalog previews + lazy Blender (2026-09-11, #32): batch sweeps take a `PreviewQuality` — Fast pins 512px/16 samples and `--engine eevee` (EEVEE's id moved across the supported range, so `setup_render` resolves it from the running build's RNA enum and falls back to Cycles at the same res/samples), Studio emits no overrides and no engine flag so its command stays byte-identical to before. The download offer moved off app launch onto the first visit to a surface that renders (`offerSetupOnce`), so browsing and scanning never mention Blender; the batch modal gained the blocked banner the Render tab already had. Rejected alongside it: a second GPU renderer (three.js or wgpu) — the scan path never touched Blender to begin with, one-launch batching already paid off the startup cost, and `StlViewport` has been rendering the LOOK rig in three.js all along
...
### Compressed-at-rest packing

After cleanup, keep each model bundled on disk (pack/unpack on demand) to save space while staying workable — the finalize_release compression path was the seed tech.
...
### 3pk writer (spec + reader: see docs/3PK.md)
...
### Minihoard integration (easter egg)
...
### Early release-builder work (0.1.x)
