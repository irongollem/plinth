...
`src/bindings.ts` is generated by the specta builder — `cargo test` rewrites
it — and **is** committed: the frontend cannot typecheck or build without
it, and neither CI's frontend job nor the release runners compile Rust
first. Commit whatever the generator emits, verbatim; CI diffs the
committed copy against a fresh one byte-for-byte and fails on any drift.
